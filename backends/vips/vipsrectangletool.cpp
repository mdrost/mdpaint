...
// public
mdpVipsRectangleTool::mdpVipsRectangleTool(mdpVipsModel& vipsModel, mdpHistory& history) :
...
// public virtual
mdpVipsRectangleTool::~mdpVipsRectangleTool() /* override */
{
}

// public virtual
void mdpVipsRectangleTool::activate() /* override */
{
    m_previewImage = m_vipsModel.getPreviewImage();
    onVipsModelPreviewResetConnection = m_vipsModel.onPreviewReset(std::bind(&mdpVipsRectangleTool::onVipsModelPreviewReset, this));
}

// public virtual
void mdpVipsRectangleTool::deactivate() /* override */
{
...
// public virtual
void mdpVipsRectangleTool::mousePressEvent(const int x, const int y) /* override */
{
    m_drawing = true;
...
// public virtual
void mdpVipsRectangleTool::mouseReleaseEvent(const int x, const int y) /* override */
{
    if (m_drawing) {
        //m_history.push(new mdpVipsHistoryEntry("Rectangle", m_vipsModel, m_previewImage));
        m_vipsModel.submit();
        m_drawing = false;
    }
}
...
// public virtual
void mdpVipsRectangleTool::mouseMoveEvent(const int x, const int y) /* override */
{
    m_vipsModel.beginDrawing();
    m_vipsModel.refresh();
    if (m_drawing) {
...
// public virtual
void mdpVipsRectangleTool::enterEvent() /* override */
{
}
...
// private
void mdpVipsRectangleTool::draw(const int x, const int y)
{
...
// private
void mdpVipsRectangleTool::draw(const int fromX, const int fromY, const int toX, const int toY)
{
    VipsImage* const previewImage = m_previewImage;
    const int imageWidth = vips_image_get_width(previewImage);
    const int imageHeight = vips_image_get_height(previewImage);
    ...
    if (width == 0 || height == 0) {
        ...
        const double exactFromX = std::min(fromX, toX) + 0.5;
        const double exactFromY = std::min(fromY, toY) + 0.5;
        char svg[SVG_RECTANGLE_SIZE];
        int snprintfResult = std::snprintf(svg, sizeof(svg), svgRectangleTemplate, imageWidth, imageHeight, exactFromX, exactFromY, width, height);
        if (snprintfResult < 0 || snprintfResult >= sizeof(svg)) {
            throw std::runtime_error("snprintf");
        }
        if (vips_svgload_string(svg, &svgImage, nullptr)) {
...
// private slot
void mdpVipsRectangleTool::onVipsModelPreviewReset()
{
    m_previewImage = m_vipsModel.getPreviewImage();
}
